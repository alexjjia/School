...
/*************************************************************************/
static int digit() //returns the register with a digit 'loaded' into it. 
{		   // uses the LOADI func.
	int reg;
...
static int variable() //returns the register with a variable 'loaded' into it.
{		      //uses the LOADAI func.
	int reg;
...
static void assign() //does the actual 'storing' of values into variables. Goes hand in hand with the variable() function.	
{		     //needs STOREAI.
	int offset, expr_reg, reg;
	switch (token) {
	
	case 'a':
		offset = (token-'a')*4;
		next_token();
		if(token != '=') //checks if next token after variable name is an assignment operator.
		{
		...
		expr_reg = expr();
		reg = 0;
		CodeGen(STOREAI, expr_reg, reg, offset);
		break;
...
static void print() //literally just outputs the registry containing said variable.
{
	int reg, offset;
	if(token != '#') {
		ERROR("Symbol%c unknown\n", token);
	...
	next_token();
	offset = (token-'a')*4;
	reg = 0; //should replace the variable() call.
	CodeGen(OUTPUTAI, reg, offset, EMPTY_FIELD);
	next_token();
...
static void morestmts()
{
	...
	default:
		break;
		//does epsilon stuffs, i.e, nothing.

	}
}

static void stmtlist() //these are void funcs, so I don't HAVE to return anything :-D :D :D!
{
	stmt();
	morestmts();
}
